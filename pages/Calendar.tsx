    ...
    <div className="space-y-6 relative h-full">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={currentEvent.id ? 'Edycja' : 'Nowe Wydarzenie'}>
        <div className="space-y-5">
            ...
            {!currentEvent.isTaskLinked && (
                <div className="flex justify-end pt-6 border-t border-white/10 mt-4 gap-3">
                    <button onClick={() => setIsModalOpen(false)} className="px-6 py-2.5 rounded-xl text-slate-400 hover:text-white hover:bg-white/5 transition-all text-sm font-bold">Anuluj</button>
                    <button onClick={handleSave} className="bg-gradient-to-r from-cyan-600 to-blue-600 text-white px-8 py-2.5 rounded-xl hover:from-cyan-500 hover:to-blue-500 font-bold shadow-lg shadow-cyan-900/20 transition-all border border-white/10">Zapisz</button>
                </div>
            )}
        </div>
      </Modal>
